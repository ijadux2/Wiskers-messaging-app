...
function insertSampleData() {
    ...
    db.run(`INSERT INTO user (id, name, bio, avatar, connection_key, created_at) VALUES (?, ?, ?, ?, ?, ?)`,
        [userId, CONFIG.PROFILE_NAME, CONFIG.PROFILE_BIO, `https://api.dicebear.com/7.x/catppuccin/svg?seed=${userId}`, connectionKey, new Date().toISOString()]
    );
    
    const sampleChats = [
        { id: 'chat1', name: 'Whiskers', avatar: 'https://api.dicebear.com/7.x/catppuccin/svg?seed=whiskers', type: 'direct', key: 'WHISK-1234', is_online: 1 },
        { id: 'chat2', name: 'Mittens', avatar: 'https://api.dicebear.com/7.x/catppuccin/svg?seed=mittens', type: 'direct', key: 'WHISK-5678', is_online: 0 },
        { id: 'chat3', name: 'Shadow', avatar: 'https://api.dicebear.com/7.x/catppuccin/svg?seed=shadow', type: 'direct', key: 'WHISK-9012', is_online: 1 }
    ];
    
    sampleChats.forEach(chat => {
        db.run(`INSERT INTO chats (id, name, avatar, type, key, is_online, created_at, unread) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
            [chat.id, chat.name, chat.avatar, chat.type, chat.key, chat.is_online, new Date().toISOString(), 0]
        );
    });
    ...
    const sampleMessages = [
        ...
        { chat_id: 'chat1', text: 'Nice! Let me know if you need anything.', is_sent: 0 }
    ];
    
    sampleMessages.forEach(msg => {
        db.run(`INSERT INTO messages (id, chat_id, text, sender_id, is_sent, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [generateId(), msg.chat_id, msg.text, msg.is_sent ? userId : 'other', msg.is_sent, 'read', new Date().toISOString()]
        );
    });
    ...
    sampleBlogs.forEach(blog => {
        db.run(`INSERT INTO blogs (id, title, content, image, author_id, author_name, author_avatar, tags, likes, comments, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [generateId(), blog.title, blog.content, `https://api.dicebear.com/7.x/catppuccin/svg?seed=${generateId()}`, userId, CONFIG.PROFILE_NAME, `https://api.dicebear.com/7.x/catppuccin/svg?seed=${userId}`, blog.tags, Math.floor(Math.random() * 50), 0, new Date().toISOString()]
        );
    });
...
function loadMessages() {
    state.chats.forEach(chat => {
        const result = db.exec(`SELECT * FROM messages WHERE chat_id = '${chat.id}' ORDER BY created_at ASC`);
        if (result.length > 0) {
...
function updateProfileUI() {
    ...
    document.getElementById('settingsKey').textContent = state.connectionKey || CONFIG.CONNECTION_KEY;
}
...
function renderChatsList() {
    ...
    document.getElementById('chatBadge').textContent = state.chats.reduce((sum, c) => sum + (c.unread || 0), 0);
}
...
function renderMyPosts() {
    ...
    const myBlogs = state.blogs.filter(b => b.authorId === state.currentUser?.id);
...
function openChat(chat) {
    ...
    document.getElementById('chatDetailStatus').textContent = chat.online ? 'Online' : 'Offline';
...
function simulateReply() {
    ...
    const replies = [
        "That's awesome! 😺", "I agree!", "Cool!", 
        "Haha funny!", "Sure thing!", "Thanks! 😄"
    ];
...
function saveProfile() {
    ...
    db.run(`UPDATE user SET name = ?, bio = ? WHERE id = ?`, [name, bio, state.currentUser.id]);
    saveDatabase();
...
function updateAvatar(event) {
    ...
    if (file) {
        ...
        reader.onload = (e) => {
            ...
            db.run(`UPDATE user SET avatar = ? WHERE id = ?`, [avatar, state.currentUser.id]);
            saveDatabase();
...
function createPost() {
    ...
    const blog = {
        ...
        title,
        content,
        image: postCoverImage || `https://api.dicebear.com/7.x/catppuccin/svg?seed=${generateId()}`,
    ...
    db.run(`INSERT INTO blogs (id, title, content, image, author_id, author_name, author_avatar, tags, likes, comments, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [blog.id, blog.title, blog.content, blog.image, blog.authorId, blog.authorName, blog.authorAvatar, blog.tags.join(','), blog.likes, blog.comments, blog.createdAt]
    );
...
function startNewChat() {
    ...
    db.run(`INSERT INTO chats (id, name, avatar, type, key, is_online, created_at, unread) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [newChat.id, newChat.name, newChat.avatar, newChat.type, newChat.key, newChat.is_online ? 1 : 0, newChat.createdAt, 0]
    );
...
function showSettingsSection(section) {
    document.querySelectorAll('.settings-option').forEach(btn => btn.classList.remove('active'));
...
function updateSettings() {
    ...
    document.getElementById('dataMessages').textContent = Object.values(state.messages).flat().length;
}
